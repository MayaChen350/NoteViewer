...
void save_chars(wchar_t current_chunk[CHUNK_SIZE / sizeof(wchar_t)],
                int string_size, sequence_t *stack_of_chunks,
                sequence_t **seq_ptr);

enum reading_state_t { UNDEFINED, READING_TITLE, SKIP, SAVE };
...
enum reading_state_t
    __attribute__((section(".text"))) *const reading_state_ptr =
        &reading_statee;

void save_chars(wchar_t current_chunk[CHUNK_SIZE / sizeof(wchar_t)],
                const int string_size, sequence_t *stack_of_chunks,
                sequence_t **seq_ptr) {
      ...
      case UNDEFINED:
        if (curr_char == '[') {
          if (*reading_state_ptr == SAVE) {
            save_chars(current_chunk, string_size, stack_of_chunks,
                       curr_seq_ptr);
          }
          ...
          *reading_state_ptr = READING_TITLE;
          ...
          string_size = 0;
        } else if (*reading_state_ptr == SAVE)
          string_size++;
        ...
        if (curr_char == ']') {
          ...
          if (string_size <= wcslen(word_to_base_output_on)) {
            ...
            *reading_state_ptr = SKIP;
            ...
            // resetting this index will be enough since it will overwrite the
            // earlier string plus, this string is only compared to its right
            // length
          }
        }
        break;
